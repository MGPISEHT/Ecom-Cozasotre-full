    ...
    try {
        ...
        foreach ($cart as $product) {
            ...
            if ($product_id > 0 && $product_name && $product_price !== false && $product_quantity > 0) {
                $stmt_items = $conn->prepare("INSERT INTO order_items (order_id, product_id, product_name, product_image, product_price, product_quantity)
                                                VALUES (?, ?, ?, ?, ?, ?)");
                $stmt_items->execute([$order_id, $product_id, $product_name, $product_image, $product_price, $product_quantity]);
            } else {
...
// Store payer info from form submission 
if (isset($_POST['username']) && !isset($_POST['action'])) {
    $_SESSION['payer_info'] = [
        'username' => filter_input(INPUT_POST, 'username', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '',
        'email' => filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL) ?: '',
        'phone' => filter_input(INPUT_POST, 'phone', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '',
        'pin_code' => filter_input(INPUT_POST, 'pin-code', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '',
        'address' => filter_input(INPUT_POST, 'address', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '',
    ];
}

// Function to retrieve payer information from session
function getPayerInfo(): array
{
    return $_SESSION['payer_info'] ?? [];
}

// --- 5. Prepare Data for HTML Display ---
$cart = getCart();
                ...
                <div class="bor10 p-lr-40 p-t-30 p-b-40 m-l-63 m-r-40 m-lr-0-xl p-lr-15-sm">
                    ...
                    <div class="flex-w flex-t bor12 p-t-15 p-b-30">
                        ...
                        <form method="post" action="place_order.php" id="checkout-form">
                            <?php $payerInfo = getPayerInfo(); // Get previously saved payer info 
                            ?>
                            <div class="bor8 bg0 m-b-22">
                                <input class="stext-111 cl8 plh3 size-111 p-lr-15" id="checkout-username" type="text" name="username" placeholder="Username" value="<?php echo htmlspecialchars($payerInfo['username'] ?? ''); ?>" required>
                            </div>

                            <div class="bor8 bg0 m-b-22">
                                <input class="stext-111 cl8 plh3 size-111 p-lr-15" id="checkout-email" type="email" name="email" placeholder="Email Address" value="<?php echo htmlspecialchars($payerInfo['email'] ?? ''); ?>" required>
                            </div>

                            <div class="bor8 bg0 m-b-22">
                                <input class="stext-111 cl8 plh3 size-111 p-lr-15" id="checkout-phone" type="tel" name="phone" placeholder="Phone" value="<?php echo htmlspecialchars($payerInfo['phone'] ?? ''); ?>" required>
                            </div>

                            <div class="bor8 bg0 m-b-22">
                                <input class="stext-111 cl8 plh3 size-111 p-lr-15" id="checkout-city" type="text" name="city" placeholder="City" value="<?php echo htmlspecialchars($payerInfo['city'] ?? ''); ?>" required>
                            </div>

                            <div class="bor8 bg0 m-b-22">
                                <input class="stext-111 cl8 plh3 size-111 p-lr-15" id="checkout-address" type="text" name="address" placeholder="Address" value="<?php echo htmlspecialchars($payerInfo['address'] ?? ''); ?>" required>
                            </div>
